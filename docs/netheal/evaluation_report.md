...
在三类故障上构造 6 种扰动配置，每种配置、每个场景使用 20 个固定随机种子，共 360 个可复现样本。扰动包括外部告警噪声、告警缺失、KPI 缺失、混合退化和严重退化。

| 扰动配置 | 样本数 | NetHeal 准确率 | 单告警基线 | 人工复核率 | 自动决策准确率 |
|---|---:|---:|---:|---:|---:|
| clean | 60 | 100.00% | 75.00% | 0.00% | 100.00% |
| alarm_noise_25pct | 60 | 100.00% | 41.67% | 0.00% | 100.00% |
| missing_alarm | 60 | 100.00% | 63.33% | 0.00% | 100.00% |
| missing_kpi | 60 | 100.00% | 78.33% | 0.00% | 100.00% |
| mixed_degradation | 60 | 98.33% | 48.33% | 1.67% | 100.00% |
| severe_degradation | 60 | 86.67% | 28.33% | 63.33% | 90.91% |
| **总体** | **360** | **97.50%** | **55.83%** | **10.83%** | **99.38%** |

“自动决策准确率”只统计未触发低置信度人工复核的样本。评测表明，在严重证据退化时，系统会主动降低自动化等级，而不是把不确定结论直接送入修复执行。
...
- `work_space/netheal_evaluation/robustness_evaluation.json`
- `work_space/netheal_evaluation/robustness_evaluation.md`

## 结果限制

功能基准和鲁棒性基准都来自本仓库合成数据，不能外推为真实运营商网络准确率。文档和答辩必须说明数据来源、扰动方式与样本规模，不能声称生产效果。
